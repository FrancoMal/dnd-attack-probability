#!/usr/bin/env node
/*
 * Genera calculator-standalone.html a partir de index.html + styles.css + engine.js + profiles-io.js + calculator.js.
...
const profilesIO = read('profiles-io.js');
...
let out = html
    ...
    .replace(/<script src="engine\.js"><\/script>\s*\n\s*<script src="profiles-io\.js"><\/script>\s*\n\s*<script src="calculator\.js"><\/script>/,
        () => `<script>\n${safeJs(engine)}\n${safeJs(profilesIO)}\n${safeJs(app)}\n    </script>`);

const header = `<!-- ARCHIVO GENERADO por scripts/build-standalone.js a partir de index.html, styles.css, engine.js, profiles-io.js y calculator.js. No editar a mano: editá los fuentes y corré "npm run build:standalone". -->\n`;
out = out.replace(/^<!DOCTYPE html>\s*\n/i, (m) => m + header);

if (!out.includes('DnDEngine') || !out.includes('DnDProfilesIO') || !out.includes('class DnDCalculator')) {
